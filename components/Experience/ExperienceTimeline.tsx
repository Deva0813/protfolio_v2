        ...
        {props.list.flat().map((exp, index) => (
          ...
          >
            <p className="font-mono text-(--text-dim) text-sm relative">
              <span
                className={clsx(
                  "block size-4 border-2 rounded-full border-(--accent-strong) bg-(--bg) absolute -left-8.25 top-1/2 -translate-y-1/2",
                  index == 0 &&
                    " ring-4 ring-(--accent-dim) bg-(--accent-strong)! ",
                )}
              ></span>
              <span className=" lowercase bg-(--gold-dim) text-(--gold) text-xs rounded-md px-2.5 py-1 ">
                {index == 0
                  ...
                  : exp.type == "full-time"
                    ? `v${exp.version}.0`
                    : empTypeConstant[exp.type]}
              </span>{" "}
              {exp.start_date_string} - {exp.end_date_string}{" "}
            </p>
            ...
            <ul className="list-disc space-y-2! text-(--text-muted) text-sm list-inside ">
              {exp.description_points.map((point, index) => (
                <li
                  key={index}
                  className="mb-1 list-none grid grid-cols-[auto_1fr] max-w-4xl"
                >
                  {" "}
                  <Plus
                    size={12}
                    className="mt-1 mr-2 text-(--accent-strong)"
                  />{" "}
                  {point}
                </li>
              ))}
