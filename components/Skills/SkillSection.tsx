          ...
          {skillsConstants.map((skill, index) => {
            return (
              <div className="" key={index}>
                <h3 className="text-xs font-medium font-mono uppercase text-(--gold)">
                  {skill.title}
                </h3>
                  ...
                  {skill.skills.map((s, i) => {
                    return (
                      <MotionSpan
                        ...
                        key={i}
                        className="rounded-sm px-3 py-1 border border-(--border) bg-(--accent-dim) text-[.8rem] text-(--text-muted)"
                      >
                        {s}
                      </MotionSpan>
                    );
